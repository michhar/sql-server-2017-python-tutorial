...
Modified and renamed by Micheleen Harris 6/21/2017
...
$csvfilename = Read-Host -Prompt 'Input the name of the csv file you want to upload to the database'
$db_tb = $dbname + ".dbo.airlinedemo"
...
try
{
	if($u -and $p)
	{
    bcp $db_tb in $csvfilepath -t ',' -S $server -f $csvfilename -F 2 -C "RAW" -b 200000 -U $u -P $p
	}
	#The MS SQL Server user and password is not specified - using the Windows user credentials 
	else 
	{
	bcp $db_tb in $csvfilepath -t ',' -S $server -f $csvfilename -F 2 -C "RAW" -b 200000 -T
	}
}
catch
{
